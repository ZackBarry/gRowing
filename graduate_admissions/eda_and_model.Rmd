...
```{r}
cor_matrix <- round(cor(select(dataset, -research)), 2)
...
To get a sense for how the new variables created by PCA are able to explain variation in the admission chances better than any two of the non-transformed variables, consider the plots below.  The top plot has `cpga` and `gre_score` plotted with`chance_of_admit` colored (these were the two variables most highly correlated with the response); the bottom plot as the first two PCA variables plotted with `chance_of_admit` colored.
```{r}
pca_values <- predict(pca_set, newdata = to_pca)
pca_values <- cbind(pca_values, select(dataset, "chance_of_admit"))

p1 <- ggplot(cbind(to_pca, select(dataset, "chance_of_admit")), aes(x = cgpa, y = toefl_score, color = chance_of_admit)) +
  geom_point() +
  theme_bw()
p2 <- ggplot(pca_values, aes(x = PC1, y = PC2, color = chance_of_admit)) +
  geom_point() + 
  theme_bw()
