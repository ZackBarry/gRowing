...
err_rand_forest <- function(cv_data, mtry, trees, min_n) {
  ...
  random_forest <- rand_forest(mode = "classification") %>%
    set_engine("ranger") %>%
    fit(Survived ~ . - TicketNumber, data = analysis(cv_data))
  
  predictions <- random_forest %>%
    predict(new_data = assessment(cv_data)) %>%
    bind_cols(new_data %>% select(Survived)) %>%
    rename(predicted = .pred_class)
  
  error_rate <- mean(predictions$predicted != assessment(cv_data)$Survived)
