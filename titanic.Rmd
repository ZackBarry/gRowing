...
prep_titanic <- function(data) {
  ...
  data <- recipe(Survived ~ ., data = data) %>%
    step_dummy(Embarked, Title) %>%
    prep(training = data, retain = T) %>%
    juice()
...
```{r}
mice_rf <- function(train, mtry, trees, min_n) {
  # train: A mice object containing train$m imputed data sets.
...
# trees: 10 - 100
err_rand_forest <- function(cv_data, mtry, trees, min_n) {
  ...
  train <- analysis(cv_data)
  test <- assessment(cv_data)
  
  train <- prep_titanic(train)
  test <- prep_titanic(test)
  
  models <- mice_rf(train, mtry, trees, min_n)
  
  result <- NULL
  
  for (i in 1:test$m) {
    test_i <- complete(test, i) 
  ...
  error_rate <- mean(ranked_choice_result$predicted != true_result$Survived)
}
...
small_train <- vfold_data$splits[[1]] %>% analysis()
small_test <- vfold_data$splits[[1]] %>% assessment()

cv_data <- vfold_data$splits[[1]]
```
