...
train <- train %>%
  mutate(
    Family_Size = ifelse(SibSp + Parch == 0, "Singleton",
                         ifelse(SibSp + Parch <= 3, "Small_Family",
                                "Large_Family"))
  )
```
...
dummy_recipe <- recipe(Survived ~ ., data = train) %>%
  step_dummy(Embarked, Title, Cabin_Letter, Family_Size) %>%
...
Now the order of steps gets a bit complicated here.  We're 
