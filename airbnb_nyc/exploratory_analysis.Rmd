...
```{r}
# don't include neighbourhood_group in model - highly correlated with neighborhood
to_model <- dataset %>%
  select(neighbourhood, latitude, longitude, room_type, price,
         minimum_nights, number_of_reviews, reviews_per_month,
         availability_365, days_since_review, id)
...
train_h2o["neighbourhood"] <- as.factor(train_h2o["neighbourhood"])
train_h2o["room_type"] <- as.factor(train_h2o["room_type"])

test_h2o <- as.h2o(test)
test_h2o["neighbourhood"] <- as.factor(test_h2o["neighbourhood"])
test_h2o["room_type"] <- as.factor(test_h2o["room_type"])

response <- "room_type"
features <- setdiff(colnames(train_h2o), c(response, "id"))
...
h2o.performance(h2o_gbm, test_h2o)
```
...
dmy <- dummyVars("room_type ~ .", data = train, fullRank = T)  # fullRank = T makes it so that there are 4 columns for a 5 level variable

train_dmy <- data.frame(predict(dmy, newdata = train))  # this leaves out room_type
train_dmy <- cbind(train_dmy, select(train, room_type))
...
train_balanced <- rbind(train_dmy, new_samples)

train_balanced_h2o <- as.h2o(train_balanced)
train_balanced_h2o["room_type"] <- as.factor(train_balanced_h2o["room_type"])

response <- "room_type"
features <- setdiff(colnames(train_balanced_h2o), c(response, "id"))

h2o_balanced_xgb <- h2o.xgboost(
  x = features,
  y = response,
...
h2o.varimp_plot(h2o_balanced_xgb)
