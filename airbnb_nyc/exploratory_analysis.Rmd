...
get_unlogged_cv_mse <- function(model, true_result_vec) {
  ...
  names(cv_df) <- paste0(names(cv_df), "_", 1:ncol(cv_df))

  # Get the fold assignment of each CV set so that the rows that are not in the hold out can be set to NA.  Note: the fold assignment of column i is i - 1.
...
Baseline model MSE, RMSE, and R^2:
```{r}
mean_log_price_train <- mean(train$log_price)
pred_log_price <- rep(mean_log_price_train, nrow(test))
true_log_price_test  <- test$log_price

sprintf("Baseline MSE: %f", mse_unlog(pred_log_price, true_log_price_test))
sprintf("Baseline RMSE: %f", rmse_unlog(pred_log_price, true_log_price_test))
sprintf("Baseline R^2: %f", rsquared_unlog(pred_log_price, true_log_price_test))
```
...
We'll focus on tuning learning rate, number of trees, number of terminal nodes, and the L1 regularization.  To start with, `ntrees` is set to 200 and the L1 term (`reg_alpha`) is set to 0 (the default value); telescopic search is applied to learning rate alongside evenly spaced values for max depth.
```{r}
ntrees <- 200
reg_alpha <- 0

hyper_parameters <- list(
  learn_rate = c(0.0001, 0.001, 0.01, 0.1, 1),
...
write_csv(results, "020620_learn_rate_and_max_depth_cv_error.csv")
```

The plots below show that a learn rate of 0.1 results in models with consistently lower cross-validated test MSE and R^2 values, regardless of the max depth.  Learn rates of 0.001 and 0.00001 perform consistently worse. Also, as max depth increases, the performance tends to decline.  Next, we'll fine tune max depth for a fixed learning rate of 0.1 before testing different values of the L1 regularization parameter.
```{r}
...
p2 <- ggplot(results, aes(x = max_depth, y = r2, color = factor(learn_rate))) +
  geom_point() +
  geom_line() +
  ggthemes::theme_tufte() +
  labs(title = "R^2 vs. Max Depth", x = "Max Depth", y = "R^2") 
...
results_2 <- do.call(
  ...
  lapply(grid@model_ids, function(x) {
    model <- h2o.getModel(x)
    h2o_pred <- h2o.predict(model, test_h2o)
    data.frame(
      mse = mse_unlog(h2o_pred, true_log_price),
      r2 = rsquared_unlog(h2o_pred, true_log_price),
      max_depth = model@allparameters$max_depth
    )
  })
)
...
p2 <- ggplot(results_2, aes(x = max_depth, y = r2)) +
  geom_point() +
  geom_line() +
  ggthemes::theme_tufte() +
  labs(title = "R^2 vs. Max Depth", x = "Max Depth", y = "R^2") 
...
We were able to use XGBoost to predict rental prices much more effectively than the baseline model.  Before hyperparameter tuning, the MSE and R^2 values were XXX and YYY; tuning the learning rate, depth, and L1 regularization parameters led to final MSE and R^2 values of XXX and YYY.  The most important predictors turned out to be AAA and BBB, which makes sense because __________.  It would be interesting to create features using the names of the listings to see if name has a noticeable impact on the predictive accuracy of the model.
...
get_unlogged_cv_mse <- function(model, true_result_vec) {
  ...
  # Get the fold assignment of each CV set so that the rows that are not in the hold out can be set to NA.  Note: the fold assignment of column i is i - 1.
  fold_assignment_vec <- as.vector(h2o.cross_validation_fold_assignment(model))
  for (i in 1:ncol(cv_predictions)) {
    cv_df[, i][fold_assignment_vec != i - 1] <- NA
  }
  
  # Calculate the difference between the hold out predictions and the true training result, removing the log transormation beforehand.
  for (i in 1:ncol(cv_predictions)) {
    cv_df[, predict_col_number] <- exp(df[, predict_col_number]) - exp(true_result_vec)
  }
